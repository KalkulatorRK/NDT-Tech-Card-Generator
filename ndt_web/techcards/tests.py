...
class SchemeDisplayTests(TestCase):
    ...
    def test_scheme_5d_preview_uses_simplified_image(self):
        ...
        ui = get_scheme_ui_data()['5d']
        self.assertIn('scheme_5d.png', ui['img'])
        self.assertNotIn('scheme_5d_docx', ui['img'])
        self.assertEqual(SCHEME_IMAGES['5d'], 'img/scheme_5d.png')
        self.assertEqual(SCHEME_DOCX_IMAGES['5d'], 'img/scheme_5d_docx.jpg')

    def test_scheme_5d_docx_image_path(self):
        """В DOCX встраивается подробный исходник схемы 3д."""
        from normative.calculations import SCHEME_INFO
...
class TemplateCommentsTests(TestCase):
    ...
    def test_scheme_5d_embeds_image_in_section_69(self):
        """Схема 3д — подробный JPEG встраивается в DOCX в п. 6.9."""
        from techcards.generator import (
            generate_from_template, get_default_template_path,
            _find_paragraph_index, _paragraph_has_drawing,
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            from django.conf import settings
            static_root = str(settings.STATICFILES_DIRS[0])
            docx_source = os.path.join(static_root, 'img', 'scheme_5d_docx.jpg')
            source_hash = hashlib.sha256(open(docx_source, 'rb').read()).hexdigest()
            generate_from_template(params, template, out, static_root=static_root)
            doc = Document(out)
            idx = _find_paragraph_index(doc, '6.9')
            ...
            self.assertIsNotNone(image_para, 'Изображение схемы не встроено в DOCX')
            caption = doc.paragraphs[idx + 3].text
            self.assertIn('Схема 3 д', caption)
            self.assertIn('ГОСТ Р 50.05.07-2018', caption)
            with zipfile.ZipFile(out) as zf:
                media = [n for n in zf.namelist() if n.startswith('word/media/')]
                embedded_hashes = [
