...
# ------------------------------------------------------------------
# ПРИЛОЖЕНИЕ Г (справочное). Выбор расстояний от источника излучения
# до контролируемого сварного соединения и длины/количества участков.
# Источник: ГОСТ Р 50.05.07-2018, стр. 33–35 (Техэксперт).
# ------------------------------------------------------------------

# Обозначения (п. Г.1, Г.2):
#   h  — радиационная толщина контролируемого участка, мм;
#   Φ  — максимальный размер фокусного пятна источника излучения, мм;
#   K  — требуемая чувствительность контроля, мм;
#   D, d — наружный и внутренний диаметры контролируемого сварного соединения, мм;
#   R_t — расчётная высота углового шва, мм;
#   D* — условный наружный диаметр (= d + 2·R_t).
#
# Дословные формулы таблицы Г.1 (извлечены OCR из RTF-исходника ГОСТ Р 50.05.07-18):
TABLE_G1_FORMULAS = {
    'G.1_base': 'f ≥ c·h',
    'G.1_L_max': 'L ≤ 0,8·f',
    'G.1_c': 'c = 2·Φ/K  (при Φ/K ≥ 2);  c = 4  (при Φ/K < 2)',
    '3a': 'l ≥ 0,7·c·(D−d)',
    '3b': 'l ≥ 0,5·c·D',
    '3g': 'l ≥ 0,5·[1,5·c·(D−d)−D]',
    '3d': 'l ≥ 0,5·[c·(1,4·D−d)−D]',
    '4a': 'l ≥ 0,7·c·(2·R_t + d)',
    '4b': 'l ≥ 0,5·[1,5·c·(D*−d)−D*],  где D* = d + 2·R_t',
    # Условия геометрической нерезкости (п. Г.2):
    'G.2_Ug': 'Φ ≤ K·d / (2·R_t)  — для схем рис. 4 (угловые швы)',
    'G.2_Ug_tube': 'Φ ≤ K·d / (D−d)  — для схем рис. 3 (кольцевые швы)',
    'G.2_D_star': 'd/D* ≥ 0,8',
}


def geometric_factor_c(focal_spot_mm: float, sensitivity_K_mm: float) -> float:
    """
    Коэффициент c по Г.1:
      c = 2·Φ/K  при  Φ/K ≥ 2
      c = 4       при  Φ/K < 2
    """
    ratio = focal_spot_mm / sensitivity_K_mm if sensitivity_K_mm else 0
    return round(2 * focal_spot_mm / sensitivity_K_mm, 3) if ratio >= 2 else 4.0


# Таблица Г.1 — расстояние l от источника до объекта контроля (не менее), мм.
# Ключи — схемы рисунка 3 (а, б, г, д) и рисунка 4 (а, б, г).
# Каждая функция принимает параметры геометрии и возвращает l, мм.
def dist_l_scheme_3a(D, d, c):
    """Рисунок 3а (кольцевой шов, панорама изнутри): l ≥ 0,7·c·(D−d)."""
    return round(0.7 * c * (D - d), 1)

def dist_l_scheme_3b(D, c):
    """Рисунок 3б (плоская деталь, одна стенка): l ≥ 0,5·c·D."""
    return round(0.5 * c * D, 1)

def dist_l_scheme_3g(D, d, Rt, c):
    """
    Рисунок 3г (кольцевой шов, источник внутри, панорама).
    По ГОСТ Р 50.05.07-2018 (Приложение Г, табл. Г.1, стр. 29 PDF):
    l ≥ 0,5·[1,5·c·(D − d) − D].
    Для гладкой трубы без усиления R_t ≈ 0 → D* = d.
    """
    return round(0.5 * (1.5 * c * (D - d) - D), 1)

def dist_l_scheme_3d(D, d, c):
    """Рисунок 3д: l ≥ 0,5·[c·(1,4·D − d) − D]."""
    return round(0.5 * (c * (1.4 * D - d) - D), 1)

def dist_l_scheme_4a(Rt, d, c):
    """Рисунок 4а (угловой шов): l ≥ 0,7·c·(2·R_t + d)."""
    return round(0.7 * c * (2 * Rt + d), 1)

def dist_l_scheme_4b(D, d, Rt, c):
    """
    Рисунок 4б (угловой шов, смещённый источник):
    l ≥ 0,5·[1,5·c·(D*−d) − D*],  где D* = d + 2·R_t.
    """
    Dstar = d + 2 * Rt
    val = 0.5 * (1.5 * c * (Dstar - d) - Dstar)
    return round(val, 1)


# Таблица Г.2 — число экспозиций N для полного контроля по схеме рис. 3а.
# Строки: d/D; столбцы: N (5..10). Значения — миним. f/D.
# Источник: ГОСТ Р 50.05.07-2018, приложение Г, стр. 26 PDF.
TABLE_G2_EXPOSURES_3A = {
    # d/D : {N: min_f/D}
    0.50: {5: None, 6: None, 7: None, 8: 14.2, 9: 3.3, 10: None},
    0.55: {5: None, 6: None, 7: None, 8: 27.3, 9: 3.4, 10: 1.8},
    0.60: {5: None, 6: None, 7: None, 8: 4.2, 9: 1.9, 10: 1.2},
    0.65: {5: None, 6: None, 7: 7.7, 8: 2.2, 9: 1.3, 10: 0.9},
    0.70: {5: None, 6: None, 7: 3.1, 8: 1.5, 9: 1.0, 10: 0.7},
    0.75: {5: 7.1, 6: 1.9, 7: 1.1, 8: 0.8, 9: 0.6, 10: None},
    0.80: {5: 3.2, 6: 1.4, 7: 0.9, 8: 0.7, 9: 0.5, 10: None},
...
# Таблица Г.3 — число экспозиций N для полного контроля по схеме рис. 3б.
TABLE_G3_EXPOSURES_3B = {
    0.40: {4: None, 5: None, 6: None, 7: None, 8: 10.4, 9: 3.2, 10: 2.0},
    0.45: {4: None, 5: None, 6: None, 7: 18.2, 8: 3.3, 9: 2.0, 10: None},
    0.50: {4: None, 5: None, 6: None, 7: 3.8, 8: 2.2, 9: 2.0, 10: None},
    0.55: {4: None, 5: None, 6: 6.9, 7: 2.8, 8: 2.0, 9: None, 10: None},
    0.60: {4: None, 5: None, 6: 4.0, 7: 2.0, 8: None, 9: None, 10: None},
    0.65: {4: None, 5: None, 6: 2.5, 7: 2.0, 8: None, 9: None, 10: None},
    0.70: {4: None, 5: 9.8, 6: 2.0, 7: None, 8: None, 9: None, 10: None},
    0.75: {4: None, 5: 4.3, 6: 2.0, 7: None, 8: None, 9: None, 10: None},
    0.80: {4: None, 5: 3.0, 6: 2.0, 7: None, 8: None, 9: None, 10: None},
    0.85: {4: None, 5: 2.3, 6: 2.0, 7: None, 8: None, 9: None, 10: None},
    0.90: {4: None, 5: 2.0, 6: None, 7: None, 8: None, 9: None, 10: None},
    0.95: {4: 18.3, 5: 2.0, 6: None, 7: None, 8: None, 9: None, 10: None},
}

# Таблица Г.4 — число экспозиций N для полного контроля по схеме рис. 3г, д.
# Столбцы: N (3..7). Значения — максим. f/D (для 3г/3д указано «не более»).
TABLE_G4_EXPOSURES_3GD = {
    0.50: {3: None, 4: 0.4, 5: 1.4, 6: 12.0, 7: None},
    0.55: {3: None, 4: 0.6, 5: 2.6, 6: None, 7: None},
}


def min_f_over_d_for_exposures(scheme: str, d_over_D: float, n_exposures: int):
    """
    Возвращает минимальное (для 3а/3б) или максимальное (для 3г/3д) отношение f/D
    по таблицам Г.2–Г.4.
    scheme: '3a' | '3б' | '3г' | '3д'.
    Возвращает (value, source) или (None, note).
    """
    table_map = {
        '3a': TABLE_G2_EXPOSURES_3A,
        '3б': TABLE_G3_EXPOSURES_3B,
        '3г': TABLE_G4_EXPOSURES_3GD,
        '3д': TABLE_G4_EXPOSURES_3GD,
    }
    table = table_map.get(scheme)
    if not table:
        return None, "Таблица для схемы %s не загружена (нужны Г.2–Г.4)." % scheme
    keys = sorted(table.keys())
    nearest = min(keys, key=lambda k: abs(k - d_over_D))
    row = table[nearest]
    val = row.get(n_exposures)
    if val is None:
        return None, f"Нет данных для d/D≈{nearest}, N={n_exposures} (таблица Г.2/Г.3/Г.4)."
    note = "не менее" if scheme in ('3a', '3б') else "не более"
    return val, f"[ГОСТ Р 50.05.07-2018, табл. Г.2/Г.3/Г.4, d/D≈{nearest}, N={n_exposures}, {note} {val}]"


def calc_source_distance_f(scheme: str, focal_spot_mm: float, sensitivity_K_mm: float,
                           D=None, d=None, Rt=0.0) -> dict:
    """
    Расчёт расстояния f (источник → контролируемое сварное соединение)
    по приложению Г ГОСТ Р 50.05.07-2018.

    :param scheme: '3a','3б','3г','3д','4a','4б'
    :param focal_spot_mm: размер фокусного пятна Φ, мм
    :param sensitivity_K_mm: требуемая чувствительность K, мм
    :param D: наружный диаметр, мм (для труб)
    :param d: внутренний диаметр, мм
    :param Rt: расчётная высота углового шва, мм
    :return: словарь с f (мм, не менее), формулой, источником
    """
    if D is None and scheme in ('3a', '3б', '3г', '3д', '4б'):
        return {'error': 'Для схемы %s нужен наружный диаметр D (мм).' % scheme}
    if d is None and scheme in ('3a', '3г', '3д', '4а', '4б'):
        return {'error': 'Для схемы %s нужен внутренний диаметр d (мм).' % scheme}
    if Rt is None and scheme in ('4a', '4б'):
        return {'error': 'Для схемы %s нужна высота углового шва R_t (мм).' % scheme}
    c = geometric_factor_c(focal_spot_mm, sensitivity_K_mm)
    calc = {
        '3a': lambda: dist_l_scheme_3a(D, d, c),
        '3б': lambda: dist_l_scheme_3b(D, c),
        '3г': lambda: dist_l_scheme_3g(D, d, Rt, c),
        '3д': lambda: dist_l_scheme_3d(D, d, c),
        '4a': lambda: dist_l_scheme_4a(Rt, d, c),
        '4б': lambda: dist_l_scheme_4b(D, d, Rt, c),
    }
    if scheme not in calc:
        return {'error': f'Схема {scheme} не поддерживается (ожидаются 3а/3б/3г/3д/4а/4б).'}
    f_val = calc[scheme]()
    return {
        'scheme': scheme,
        'f_mm_min': f_val,
        'c': c,
        'formula': 'l ≥ f ≥ c·h (Г.1); табл. Г.1 для конкретной схемы',
        'source': '[ГОСТ Р 50.05.07-2018, приложение Г, табл. Г.1]',
        'note': 'После выбора f определить f/D и по табл. Г.2–Г.4 найти N (число экспозиций).',
    }
