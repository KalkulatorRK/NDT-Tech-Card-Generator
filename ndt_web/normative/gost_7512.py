...
Базовый методический документ РГК (любые отрасли). ГОСТ Р 50.05.07-2018
дополняет его для атомной энергетики.

Модуль содержит:
- идентификатор документа;
- проволочные эталоны чувствительности (табл. 2) и маркировку (п. 2.13);
- условную запись дефектов (прил. 5–6);
- справочник подлинных обозначений схем (черт. 4, 5а–5з, 6).

Формулы f/N/L — в normative.calculations (прил. 4 ГОСТ 7512-82).
"""
...
DOCUMENT_FULL_NAME = (
    'ГОСТ 7512-82 «Контроль неразрушающий. Сварные соединения. '
    'Радиографический метод»'
)

# Подлинные обозначения чертежей схем (п. 4, прил. 4). Ключ — внутренний код расчёта.
SCHEME_DRAWING_LABELS = {
    '4_6': 'Черт. 4',
    '5a': 'Черт. 5а',
    '5b': 'Черт. 5б',
    '5v': 'Черт. 5в',
    '5g': 'Черт. 5г',
    '5d': 'Черт. 5д',
    '5zh': 'Черт. 5е',  # панорамное Dн ≤ 2 м
    '5z': 'Черт. 5з',   # выборочный Dн ≥ 2 м
    '5e': 'Черт. 5ж',   # если 5е невозможно
}
...
def resolve_iqi_placement(
    ...
    """
    Сторона установки ИКИ.

    Для АЭУ уточняется ГОСТ Р 50.05.07-2018, п. 6.1.11; для общей методики
    ГОСТ 7512-82 — предпочтительно со стороны источника (см. также п. 5.5).
    """
    cite_507 = 'ГОСТ Р 50.05.07-2018, п. 6.1.11'
    cite_7512 = 'ГОСТ 7512-82'
    use_7512 = '7512' in (doc_code or '')
    if iqi_side == SIDE_FILM:
        note = (
            'ИКИ со стороны плёнки: проволочный эталон на одну ступень '
            f'жёстче относительно требуемой чувствительности K ({cite_507}).'
            if not use_7512 else
            'ИКИ со стороны плёнки: ужесточение подбора проволоки на 1 ступень '
            f'(практика по {cite_507}; базовая методика — {cite_7512}).'
        )
        return {
            'side': SIDE_FILM,
    ...
    return {
        ...
        'shift_steps': 0,
        'note': (
            f'ИКИ устанавливается со стороны источника излучения ({cite_7512}).'
            if use_7512 else
            f'ИКИ устанавливается со стороны источника излучения ({cite_507}).'
        ),
    }
